	...
	<table width="98%" border="0" align="center" cellpadding="0" cellspacing="0" style="margin-top:12px;">
		...
		<table  width="100%" border="0" align="center" cellpadding="0" cellspacing="0" id="tb" >
			<tr class="title_font">
				...
				<td align="center" width=100% bgcolor="#C7E2F8"><span class="out">
					标题
				</span></td>			
			...
			<%
			int n = 0;
			CaseNode pn1 = new CaseNode();
			%>
			<c:forEach var="aBean" items="${pagelist}" varStatus="status">
											<%
												n++;
												pn1 = (CaseNode)pagelist.get(n-1);
											...
											<tr onmouseover=this.className='td_over' onmouseout=this.className='' id='r1' bgcolor="<%=n != n / 2 * 2 ? "#ebf3f6" : "#ebf3f6"%>"
												...
												<td  class='num_font' height="30" valign="middle" nowrap="nowrap" <c:if test="${aBean.attribute.remark!=null}">onMouseMove="javescript:disp_s('<%=n %>');" onMouseOut="javescript:disp_n('<%=n %>');"</c:if>>
												&nbsp;&nbsp;&nbsp;&nbsp;<a href="<%=request.getContextPath()%>/toViewXxzl.action?cname=case&docid=<%=pn1.getId() %>" title="" >
													<span class="">
															<font
        ...
        <tr>
          <td align="center" valign="middle" class="num_font"><div class="page1_box">
          		<form name="queryform" action="<%=request.getContextPath()%>/case/case_list.jsp" method="post">
					...
					<input type="hidden" name="orderby" value="<%=orderby%>"></input>
						...
						<c:if test="${pagelist!= null}">
							<elile:navigateBar navigateform="navigateform"
								actionName="/falnewexam/case/case_list.jsp" formName="queryform" />
						</c:if>
					</form>
          </div></td>
        </tr>
