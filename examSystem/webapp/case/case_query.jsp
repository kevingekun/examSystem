...
<%@taglib uri="http://struts.apache.org/tags-html" prefix="html"%> 
	...
	txtfilesearcher.setYe(dang);
   	txtfilesearcher.make("case",realpath,user.getUserType().toString());
              ...
              <tr>
                <td><table width="100%" border="0" align="left" cellpadding="0" cellspacing="0">
                    <tr>
                      ...
                      <td  class="header8">查询条件 <a style="font-size:12px" href=# onclick="history.back(1)">后退</a>  <a style="font-size:12px" href="case_list.jsp" >回到搜索页</a> </td>
                    </tr>
                </table></td>
              </tr>
            </table>

	<FORM id=searchForm name="searchForm" action=case_query.jsp onsubmit="return jiancha()">
	...
	约有
						...
						<% 			
								...
								%>&nbsp;<%out.print("请换用简洁的字词。");%><br><%
						 		...
						 		AbstractTree t = FactoryBean.creator("case");
											fathernode = (CaseNode)t.getNodeById(Node.getNodeType("case"),new Long(idd));
						 	...
						 	<br>
								<center>
									<%
										if(dang>1){
									%>
											<A HREF="case_query.jsp?dang=<%=dang-1%>&searchWord=<%=queryStr%>" class="fa">
									<%
											out.print("上一页");	
									%></A>
									<%
										...
										%>
												<A <%if(j==dang)out.print("style='color:#CC0000'");%> HREF="case_query.jsp?dang=<%=j%>&searchWord=<%=queryStr%>">
										<%
												out.print(j);	
										%></A>
										<%				
									 ...
									 <%
										if(dang<zongye){
									 %>
											<A HREF="case_query.jsp?dang=<%=dang+1%>&searchWord=<%=queryStr%>">
									 <%
											out.print("下一页");	
										}
									 %></A><br><br><br>
