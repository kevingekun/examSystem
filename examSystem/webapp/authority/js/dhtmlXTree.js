...
// Add by Sjm. Perhaps including some bugs.
dhtmlXTreeObject.prototype.moveBefore=function(itemObject,targetObject) {
	
	var oldTree=itemObject.treeNod;
	var c=itemObject.parentObject.childsCount;
	var z=itemObject.parentObject;
	var parentObject = targetObject.parentObject;
	var Nodes = parentObject.childNodes;
	var Count = parentObject.childsCount;
	var i;
	...
	itemObject.treeNod = parentObject.treeNod;
	parentObject.childsCount++;
	
	var tr=this._drawNewTr(Nodes[Count].htmlNode);
	parentObject.htmlNode.childNodes[0].insertBefore(tr, targetObject.tr);
	...
	itemObject.parentObject = parentObject;
	if(oldTree!=parentObject.treeNod){if(itemObject.treeNod._registerBranch(itemObject,oldTree))return;this._clearStyles(itemObject);};
...
dhtmlXTreeObject.prototype.checkLast=function(itemObject) {
	
	var i;
	var c = itemObject.parentObject.childsCount;
	var temp;
	
	for(i=0, temp = itemObject;temp;i++, temp=temp.tr.previousSibling.nodem);
