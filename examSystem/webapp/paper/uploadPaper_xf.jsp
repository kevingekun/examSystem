...
<%
 ...
 List<EPapers> p_list = professionBean.getUnUploadPapersOnPaper_xf();
	...
	if(sjid==""){
	    ...
	    if(row>0)
	    {
			...
			$.ajax({
				type: 'post',
				async: false,
				url: 'relatePaperAndTeam_xf.action?checkid='+checkedid+'&sjid='+sjid,
...
function checkPaper_xf(v){
	$("#sjid").val(v);
	//alert(v)
}
function alreadyRelated(){
	window.open ("<%=request.getContextPath() %>/paper/paper_upload_xf_alreadyRelated.jsp", "newwindow", "height=500, width=800, top=100, left=100, toolbar=no, menubar=no, scrollbars, resizable=no, location=no, status=no");
}
function alreadyUpload(){
	window.open ("<%=request.getContextPath() %>/paper/paper_upload_xf_alreadyUpload.jsp", "newwindow", "height=500, width=1000, top=100, left=100, toolbar=no, menubar=no, scrollbars, resizable=no, location=no, status=no");
}
function findRecord(v){
	window.open ("<%=request.getContextPath() %>/paper/paper_record_xf.jsp?id="+v, "newwindow", "height=500, width=1000, top=100, left=100, toolbar=no, menubar=no, scrollbars, resizable=no, location=no, status=no");
}
</script>
<script type="text/javascript">
									...
									<table width="99%" border="0" align="center" cellpadding="0" cellspacing="0">
										...
										<tr>
											...
											<td width="18%" align="left">
												...
												<select id="paper" name="paper" onchange="checkPaper_xf(this.value)">
											...
											<td height="8">
												<input type="button" class="submit_2" onClick="alreadyUpload()" value="查看已下放"/>
											</td>
										</tr>
										<tr>
											<td height="8">
							...
							<td class="borader3"><table width="100%" border="0" cellspacing="0" cellpadding="0">
								...
								<table width="100%" border="0" align="center" cellpadding="0" cellspacing="1" class="table_list" id="tb">
									<tr class="title_font">
										...
										<td width="15%" align="center" bgcolor="#C7E2F8"><span class="out">鉴定所名称</span></td>
										<td width="7%" align="center" bgcolor="#C7E2F8"><span class="out">鉴定所编号</span></td>
										...
										<td width="20%" align="center" bgcolor="#C7E2F8"><span class="out">地址</span></td>
										<td width="15%" align="center" bgcolor="#C7E2F8"><span class="out">操作</span></td>
									</tr>
									...
									<c:forEach var="aBean" items="${xfList}" varStatus="status">
										<tr onMouseOver="this.className='td_over'" onMouseOut="this.className=''" id='r1'>
											<td align='center' class='num_font'><input type="checkbox" id="checkbox" name="checkedid" value="<c:out value='${aBean.id}'/>" /></td>
											<td align='center' class='num_font'><c:out value="${status.index+1}" /></td>
											<td align='center' class='num_font'><c:out value="${aBean.station_name}" /></td>
											<td align='center' class='num_font'><c:out value="${aBean.id}" /></td>
											<td align='center' class='num_font'><c:out value="${aBean.contacts}" /></td>
											<td align='center' class='num_font'><c:out value="${aBean.contact_number}" /></td>
											<td align='center' class='num_font'><c:out value="${aBean.address}" /></td>
											<td align='center' class='num_font'><button class="BigButton" onclick='findRecord(<c:out value="${aBean.id}" />)'>下放记录</button></td>
										</tr>
									</c:forEach>
								</table> 
