...
<form method="POST" action="<%=request.getContextPath() %>/paperServlet">
<input type=hidden name="actionType" value="checkPaper">
<input type=hidden name="answerpageid" value="<%=examanswerid%>">
<TABLE border=0 align=center width=100% height=100% cellpadding=1 cellspacing=1 bgcolor="#D2E8FF">
			...
			<%
				if(rclist!=null&&rclist.size()!=0){
					...
					for(int j=0;j<rclist.size();j++){
						...
						out.println("</tr></table>");
						out.println("&nbsp;&nbsp;&nbsp;&nbsp;得分：<input type=text size=5 name='recordmark' value=0><br>");									
						k++;
					}
					out.println("</TD>"); 
					out.println("</TR>"); 
			...
			<%
				if(dllist!=null&&dllist.size()!=0){
					...
					for(int j=0;j<dllist.size();j++){
						...
						out.println("<td><textarea style='overflow:visible;color:#0000ff;border=0' name='dialogkeys'>"+eas.getCorrectkey()+"</textarea></td>");
						out.println("</tr></table>");
						out.println("&nbsp;&nbsp;&nbsp;&nbsp;得分：<input type=text name='dialogmark' size=5 value=0><br>");									
						k++;
					}
					out.println("</TD>"); 
					out.println("</TR>"); 
	...
	<tr><td align=center><a href="#" class="SmallButton" onclick="submit()">提交</a></td></tr>
</TABLE>
</form>
</BODY>
</HTML>
<%System.out.println("HibernateUtil.issessionExist at answerentitygrader.jsp=========>>>>>"+HibernateUtil.isSessionExist());%>