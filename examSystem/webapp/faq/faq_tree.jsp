...
<%
	String cname = RequestUtils.getString(request, "cname", "faq");
...
private String createMenu(Node node,String http)  {
	...
	str.append(menu + "[1] = '<tr onmouseover=javascript:this.style.background=\"#B6C9D8\" onmouseout=javascript:this.style.background=\"#E0E9F1\" onclick=doAction(\"faq_list.jsp?nodeid=" + node.getId() + "\") class=\"word10\"><td style=\"height:16px; cursor:hand\" >&nbsp;&nbsp;文档列表</td></tr>'\n");
...
function reloadWin(){
	document.location.href="<%=request.getContextPath()%>/faq/faq_tree.jsp";
}
function noright(){
	alert("抱歉，您未授权访问该栏目!");
}
function show(m, obj) {
	doAction("<%=request.getContextPath()%>/faq/faq_list.jsp?nodeid="+m);
}
...
<%if("1".equals(status)){%>
<script>
	doAction("<%=request.getContextPath()%>/faq/faq_list.jsp");
</script>
<%} %>
...
<script src="<%=request.getContextPath()%>/faq/js/ua.js"></script>
<script src="<%=request.getContextPath()%>/faq/js/ftiens4.js"></script>
<script language="javascript">
...
<form name="form1" action="<%=request.getContextPath()%>/faq/site_tree.jsp" method="get" >
<DIV id=loader_container align=left>
<DIV id=loader>
<DIV align=left><img src="<%=request.getContextPath()%>/faq/images/loading.gif"  align="absmiddle">页面正在加载中 ...</DIV>
</DIV></DIV>
<DIV>
...
menusite<%=nodeBase.getId()%>[1] = '<tr onmouseover=javascript:this.style.background="#B6C9D8" onmouseout=javascript:this.style.background="#E0E9F1" onclick=doAction("<%=request.getContextPath()%>/faq/faq_list.jsp?nodeid=<%=nodeBase.getId()%>")  class="word10"><td style="height:16px; cursor:hand" >&nbsp;&nbsp;文档列表</td></tr>'
...
PRESERVESTATE = 1
GLOBALTARGET="R"
ICONPATH = "<%=request.getContextPath()%>/faq/images/";
