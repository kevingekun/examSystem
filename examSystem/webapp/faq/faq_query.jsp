...
<link href="../inc/all.css" rel="stylesheet" type="text/css" />
	...
	txtfilesearcher.setYe(dang);
   	txtfilesearcher.make("faq",realpath,user.getUserType().toString());
              ...
              <tr>
                <td><table width="100%" border="0" align="left" cellpadding="0" cellspacing="0">
                    <tr>
                      ...
                      <td  class="header8">查询条件 <a style="font-size:12px" href=# onclick="history.back(1)">后退</a>  <a style="font-size:12px" href="faq_list.jsp" >回到搜索页</a> </td>
                    </tr>
                </table></td>
              </tr>
            </table>

	<FORM id=searchForm name="searchForm" action=faq_query.jsp onsubmit="return jiancha()">
						...
						<% 			
								...
								%>&nbsp;<%out.print("请换用简洁的字词。");%><br><%
						 		...
						 		AbstractTree t = FactoryBean.creator("faq");
											fathernode = (FaqNode)t.getNodeById(Node.getNodeType("faq"),new Long(idd));
						 	...
						 	<br>
								<center>
									<%
										if(dang>1){
									%>
											<A HREF="faq_query.jsp?dang=<%=dang-1%>&searchWord=<%=queryStr%>" class="fa">
									<%
											out.print("上一页");	
									%></A>
									<%
										...
										%>
												<A <%if(j==dang)out.print("style='color:#CC0000'");%> HREF="faq_query.jsp?dang=<%=j%>&searchWord=<%=queryStr%>">
										<%
												out.print(j);	
										%></A>
										<%				
									 ...
									 <%
										if(dang<zongye){
									 %>
											<A HREF="faq_query.jsp?dang=<%=dang+1%>&searchWord=<%=queryStr%>">
									 <%
											out.print("下一页");	
										}
									 %></A><br><br><br>
