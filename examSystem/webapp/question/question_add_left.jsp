				...
				<script type="text/javascript">
					...
					List<Tkcategory> t = professionBean.getAllTkcategories();
				  	for (Iterator<Tkcategory> it = t.iterator(); it.hasNext();) 
				  	{
				     	...
				     	//添加树的结点
				     	//out.println("t.add('"+node.getId()+"',0,'"+node.getJobname()+"'"+",'question/question_add_right.jsp?id_job="+node.getId_job()+"&gzid="+node.getId()+"','','MainWin')");
				  	...
				  	for (Iterator<Tjobsubject> it = tree.iterator(); it.hasNext();) 
				  	{
				     	...
				     	//添加树的结点
				     	//out.println("t.add('"+node.getId()+"',0,'"+node.getJobname()+"'"+",'question/question_add_right.jsp?id_job="+node.getId_job()+"&gzid="+node.getId()+"','','MainWin')");
				     	out.println("t.add('"+node.getId()+"','"+node.getCategory()+"','"+node.getJobname()+"'"+",'question/question_add_right.jsp?id_job="+node.getId_job()+"&gzid="+node.getId()+"','','MainWin')");
				  	}
