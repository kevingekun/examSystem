...
private String createMenu(Node node,String http)  {
	...
	str.append(menu + "[1] = '<tr onmouseover=javascript:this.style.background=\"#B6C9D8\" onmouseout=javascript:this.style.background=\"#E0E9F1\" onclick=doAction(\"ser_list.jsp?nodeid=" + node.getId() + "\") class=\"word10\"><td style=\"height:16px; cursor:hand\" >&nbsp;&nbsp;文档列表</td></tr>'\n");
...
<script language="javascript" src="<%=request.getContextPath()%>/service/js/menu.js"></script>
...
function reloadWin(){
	document.location.href="<%=request.getContextPath()%>/service/ser_tree.jsp";
}
function noright(){
	alert("抱歉，您未授权访问该栏目!");
}
function show(m, obj) {
	doAction("<%=request.getContextPath()%>/service/ser_list.jsp?nodeid="+m);
}
...
<%if("1".equals(status)){%>
<script>
	doAction("<%=request.getContextPath()%>/service/ser_list.jsp");
</script>
<%} %>
...
<form name="form1" action="site_tree.jsp" method="get" >
<DIV id=loader_container align=left>
<DIV id=loader>
<DIV align=left><img src="<%=request.getContextPath()%>/service/images/loading.gif"  align="absmiddle">页面正在加载中 ...</DIV>
</DIV></DIV>
<DIV>
          ...
          <table width="100%" border="0" cellspacing="0" cellpadding="0">
            <tr>
              <td height="10"><table width="100%"  border="0" cellspacing="0">
                <tr>
                  <td width="40%" height="25" bgcolor="#A2D5FF"><img src="<%=request.getContextPath()%>/service/images/open.gif">文档管理</td>
                  <td width="60%" bgcolor="#A2D5FF" align="center">
				  <a href="javascript:collapseTree(1)" title="折叠"><img src="<%=request.getContextPath()%>/service/images/min_view.gif" width="16" height="16" border="0"></a><a href="javascript:expandTree(foldersTree)" title="展开"><img src="<%=request.getContextPath()%>/service/images/max_view.gif" width="16" height="16" border="0"></a><a href="javascript:reloadWin()" title="刷新"><img src="<%=request.getContextPath()%>/service/images/reload.gif" width="13" height="16" border="0" align="absmiddle"></a><a href="javascript:closefrm()" title="关闭"><img src="<%=request.getContextPath()%>/service/images/close_view.gif" width="16" height="16" border="0" align="absmiddle" title="关闭"></a></td>
				</tr>
                <tr>
                  <td height="1" colspan="2" bgcolor="#AAC1DE"></td>
                </tr>
...
menusite<%=nodeBase.getId()%>[1] = '<tr onmouseover=javascript:this.style.background="#B6C9D8" onmouseout=javascript:this.style.background="#E0E9F1" onclick=doAction("<%=request.getContextPath()%>/service/ser_list.jsp?nodeid=<%=nodeBase.getId()%>")  class="word10"><td style="height:16px; cursor:hand" >&nbsp;&nbsp;文档列表</td></tr>'
...
PRESERVESTATE = 1
GLOBALTARGET="R"
ICONPATH = "<%=request.getContextPath()%>/service/images/";
