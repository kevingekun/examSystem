...
<%
    ...
    List nodeList1 =tree.getNavigationTree(Node.getNodeType(cname),null); 
...
function reloadWin(){
	document.location.href="ser_tree.jsp";
}
function noright(){
	alert("抱歉，您未授权访问该栏目!");
}
function show(m, obj) {
	doAction("ser_list.jsp?nodeid="+m);
}
...
menusite<%=nodeBase.getId()%>[1] = '<tr onmouseover=javascript:this.style.background="#B6C9D8" onmouseout=javascript:this.style.background="#E0E9F1" onclick=doAction("ser_list.jsp?nodeid=<%=nodeBase.getId()%>")  class="word10"><td style="height:16px; cursor:hand" >&nbsp;&nbsp;文档列表</td></tr>'
