	...
	<table width="99%"  border="0" align="center" cellpadding="3" cellspacing="1" bgcolor="#84B7C6">
		...
		<% 
			for(int i=0;i<grouplist.size();i++){
				T_bm group = (T_bm)grouplist.get(i);
		%>
		<%if(!group.getBmid().equals("1")&&!group.getBmid().equals("7")&&!group.getBmid().equals("2")){ %>
		<tr bgcolor=#F1F8FE> 
			
			<td width="763" >
			
				<input type=checkbox name="team<%=(i+1)%>" id="team<%=(i+1)%>" onclick="checkTeam(<%=(i+1)%>,this.checked)"><%=group.getBmmc() %>
		...
		<tr bgcolor="#ffffff">
			<td>
				<table border=0 width=100% cellspacing=0 cellpadding=0 >
					...
					List userset = null;
					if(!group.getBmid().equals("1")&&!group.getBmid().equals("7")&&!group.getBmid().equals("2")){
						 userset = authbean.findUserByGroup(new Long(group.getBmid()));  
						...
						if(userset!=null&&userset.size()>0){
							for(int j=0;j<(userset.toArray().length/4+1);j++){
								...
								for(int k=0;k<(j==(userset.toArray().length/4)?(userset.toArray().length%4):4);k++){
									...
									Usergroup_view user = (Usergroup_view)userset.toArray()[j*4+k];
									out.println("<td><input type=checkbox name='teamer"+(i+1)+"' value="+user.getGonghao()+"><font id='"+user.getGonghao()+"'>"+user.getRealname()+"</font></td>");
								}
