...
function goback(){
	window.location.href="<%=request.getContextPath() %>/papersServlet?actionType=queryUnpass&sjZt=5";
}
...
<script>
	...
	//审核试卷状态
  function modify(sjid,paperState){
	window.location.href="modify.action?paperid="+sjid+"&paperState="+paperState;
}
</script>
<BODY>
	     ...
	     <table width="100%" height="30" border="0" cellpadding="0" cellspacing="0">
		                   <tr>
		                     <td height="40" align="center" valign="bottom">
		                      <input  type=button class="submit_2" onClick="jvavscript:modify(<c:out value="${pager.sjId}"/>,4);"  value="组卷完成"/>&nbsp;
		                    </td>
		                  </tr>
		          </table>
          <br>
   ...
   <ul class="first_t">
	...
	<s:set name="epaperquestions" value="epaperquestions.{?#this.equestions.equestiontype.id==#priority}"/>
		...
		<ul class="second_t"><li>
					...
					if(p.getSjZt()==new Long(5).longValue()){%><input type="button" class="smallButton" onclick='deleteSt(<s:property value="#epq.id"/>,<s:property value="paperid"/>);' value="删除"></input>
				    <%} %>
				<!-- 如果是单选题或多选题则要把试题分割显示 -->
