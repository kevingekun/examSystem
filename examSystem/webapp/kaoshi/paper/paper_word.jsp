...
<BODY>
	<h1 style="font-size:24px; font-family:黑体; color:#06374A; text-align:center;"><c:out value="${pager.sjMc}"/></h1>
	<table width="90%" border="0" align="center" cellpadding="3" cellspacing="1" style="background:#eaf4fd; font-size:12px;">
          <tr bgcolor="#FFFFFF">
            <td>试卷名称 : </td>
            <td colspan="2"><c:out value="${pager.sjMc}"/></td>
            <td>试卷总分 : </td>
            <td colspan="2"><c:out value="${pager.sjZf}"/>&nbsp;&nbsp;&nbsp;(已选试题的总分：<c:out value="${fenshu}"/>)&nbsp;&nbsp;&nbsp;(已选试题的总分：<c:out value="${fenshu}"/>)</td>
          </tr>
          <tr bgcolor="#FFFFFF">
            <td width="10%">答卷时限: </td>
...
<table width="90%" border="0" align="center" cellpadding="0" cellspacing="10" style="font-size:12px;font-family:宋体; border:solid 1px #D2E8FF">								
		...
		<s:iterator value="#epaperquestions" status="index" id="epq">			
		 ...
		 <td style="font-weight:bold; padding-left:45px;">
						...
						&nbsp;.&nbsp;<s:property value="@com.wondersgroup.falcon.Util.StringUtils@convertToBrNew(#epq.equestions.stTg)" escape="false"/>&nbsp;&lt;<s:property value="#epq.sjStfs"/>分&gt;
						&nbsp;&nbsp;&nbsp;
	    ...
	    <div style="background:url(newimages/answer_bg.jpg) repeat-x #F3F6FB;border:1px solid #D5DBEF; height:23px; line-height:23px; font-size:14px; padding:0 5px; color:#555; margin-left:60px;">
	           标准答案： <s:generator separator="||" val="#epq.equestions.stDa">
										<s:iterator>
											<s:property/>、
										</s:iterator>
									</s:generator></div></td>
		...
		<s:elseif test="#priority==3" >		
	     ...
	     <td style="padding-left:60px; line-height:22px;">
		   <div style="background:#f1f1f1;padding:5px">
	           标准答案： <s:if test='#epq.equestions.stDa=="T"'>
										正确
									</s:if>
									<s:else>
										错误
	    ...
	    <tr>
	     <td style="padding-left:60px; line-height:22px;">
		   <div style="background:#f1f1f1;padding:5px">
			 ...
			 答题说明：<s:property value="@com.wondersgroup.falcon.Util.StringUtils@convertToBrNew(#epq.equestions.stDasm)" escape="false"/></div></td>
	    </tr>
		</s:elseif>		
		...
		<s:elseif test="#priority==5">
		   ...
		   <tr>
		    <td style="padding-left:60px; line-height:22px;">
		       <div style="background:url(newimages/answer_bg.jpg) repeat-x #F3F6FB;border:1px solid #D5DBEF;line-height:23px; font-size:12px; padding:0 5px; color:#555;">标准答案：<br>
             答题：<s:property value="@com.wondersgroup.falcon.Util.StringUtils@convertToBrNew(#epq.equestions.stDa)" escape="false"/></div>
	        </td>
	      </tr>
