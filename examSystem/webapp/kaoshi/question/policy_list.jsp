...
<%@taglib uri="http://struts.apache.org/tags-html" prefix="html"%> 
	...
	<script language="javascript" src="<%=request.getContextPath()%>/kaoshi/question/images/menu.js"></script>
	...
	System.out.println("dddddddd"+nodeid);
	...
	if (op.equals("query")) {
		...
		System.out.println("fdfdf"+year);
		if(year!=null&&!year.equals("")){
		attr.setYear(new Long(year));
		}
...
<SCRIPT language=JavaScript>
	...
	var self =document.getElementById("self_id").value;
...
targelem.style.visibility='hidden';
	...
	function aaaa(){
		...
		if(count==0){
			...
			//window.parent.opener.document.getElementById("ST_WH").value=values[0];
			//window.parent.opener.document.getElementById("ST_CC").value=values[1];
		}
...
-->
    ...
    <table width="99%" border="0" align="center" cellpadding="0" cellspacing="0">
    	...
    	<tr class="row_height">
          <td width="10%" height="26" align="right">标题：</td>
          <td width="10%"><input name="name" type="text" class="input3"  ></td>
          <td width="10%" align="right">文号:</td>
          <td width="10%" align="left">
          <input id="fileno" name="fileno" class="input3" type="text" value="" ></td>
          <td width="8%" align="right">年份：</td>
          <td width="8%"><input name="year" type="text" class="input3"  ></td>
          <td width="15%" align="left">
           是否已出题：<input type="checkbox" name="chuti" id="chuti" value="1" ></td>
         <td width="10%" > <INPUT class = "submit_2" id=doSearch type=submit value=搜索></INPUT></td>
	...
	<table width="98%" border="0" align="center" cellpadding="0" cellspacing="0" style="margin-top:12px;">
		...
		<table  width="100%" border="0" align="center" cellpadding="0" cellspacing="0" id="tb" >
			<tr class="title_font">
				...
				<td align="center" width=50% bgcolor="#C7E2F8"><span class="out">
					标题
				</span></td>			
				<td align="center" width=10% bgcolor="#C7E2F8"><span class="out">
					状态
				</span></td>
				<td align="center" width=10% bgcolor="#C7E2F8"><span class="out">
					选择
			...
			<c:forEach var="aBean" items="${pagelist}" varStatus="status">
											...
											<tr onmouseover=this.className='td_over' onmouseout=this.className='' id='r1'  bgcolor="<%=n != n / 2 * 2 ? "#ebf3f6" : "#ebf3f6"%>">
												...
												<c:choose>
												<c:when test="${aBean.attribute.effective == '4'}">
											<tr onmouseover=this.className='td_over' onmouseout=this.className='' id='r1'  bgcolor="<%=n != n / 2 * 2 ? "#ebf3f6" : "#ebf3f6"%>">
												</c:when>
												</c:choose>
												<c:choose>
												<c:when test="${aBean.attribute.effective == '5'}">
											<tr onmouseover=this.className='td_over' onmouseout=this.className='' id='r1'  bgcolor="<%=n != n / 2 * 2 ? "#ebf3f6" : "#ebf3f6"%>">
												</c:when>
												</c:choose>
												...
												<!--  <a href="<%=request.getContextPath()%>/toview_node.action?cname=policy&docid=<%=pn1.getId() %>" title="<%=request.getContextPath()%>/toview_node.action?cname=policy&docid=<%=pn1.getId() %>" >-->
												<a href="\\168.30.1.54\c$\Program Files\IBM\WebSphere\AppServer\profiles\AppSrv01\installedApps\yibao-cd3ed4aedNode01Cell\falnew_war.ear\falnew.war\libstore\policy\<%=pn1.getAttribute().getHtmlFile() %>" title="\\168.30.1.54\c$\Program Files\IBM\WebSphere\AppServer\profiles\AppSrv01\installedApps\yibao-cd3ed4aedNode01Cell\falnew_war.ear\falnew.war\libstore\policy\<%=pn1.getAttribute().getHtmlFile() %>" target="_blank" >
									<span class="">
															<font
											...
											<td class='num_font' align="center" valign="middle"  >
													<c:choose>
														...
														<c:when test="${aBean.attribute.effective == '4'}">
															<font color="black">修订前</font>
														</c:when>
														<c:when test="${aBean.attribute.effective == '5'}">
															<font color="black">修订后</font>
														</c:when>
													</c:choose>
												</td>
												<td class='num_font' align="center" valign="middle"  >
													<input type=checkbox name="zhengchefagui" value='<c:out value="${aBean.attribute.fileno}"/>||<c:out value="${aBean.name}" />||<c:out value="${aBean.id}" />'/>
												</td>
											</tr>
											<%} %>
										</c:forEach>
        ...
        <tr>
          <td align="center" valign="middle" class="num_font"><div class="page1_box">
          		<form name="queryform" action="<%=request.getContextPath()%>/kaoshi/question/policy_list.jsp" method="post">
						...
						<c:if test="${pagelist!= null}">
							<elile:navigateBar navigateform="navigateform"
								actionName="/falnewexam/kaoshi/question/policy_list.jsp" formName="queryform" />
						</c:if>
					</form>
          </div></td>
        </tr>
