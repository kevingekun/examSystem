...
<%@taglib uri="http://struts.apache.org/tags-html" prefix="html"%> 
	...
	String[] filter = new String[]{"q","w","e","r","t","y","u","i","o","p","a","s","d","f","g","h","j","k","l","z","x","c","v","b","n","m"}; 
	...
	txtfilesearcher.setYe(dang);
   	txtfilesearcher.make("policy",realpath,user.getUserType().toString());
              ...
              <tr>
                <td><table width="100%" border="0" align="left" cellpadding="0" cellspacing="0">
                    <tr>
                      ...
                      <td  class="header8">查询条件 <a style="font-size:12px" href=# onclick="history.back(1)">后退</a>  <a style="font-size:12px" href="policy_list.jsp" >回到搜索页</a> </td>
                    </tr>
                </table></td>
              </tr>
            </table>
	...
	约有
						...
						<% 			
								...
								%>&nbsp;<%out.print("请换用简洁的字词。");%><br><%
						 		...
						 		AbstractTree t = FactoryBean.creator("policy");
											fathernode = (PolicyNode)t.getNodeById(Node.getNodeType("policy"),new Long(idd));
							...
							%> 
								<table  width=80% border=0>
									<tr>
										...
										<td>
											<a href="law_file.jsp?fileid=<%=txtfilesearcher.getList().get(i).toString().substring(txtfilesearcher.getList().get(i).toString().lastIndexOf("/")+1,txtfilesearcher.getList().get(i).toString().length()) %>&filename=<%=txtfilesearcher.getList3().get(i).toString() %>&selectitem=<%=txtfilesearcher.getList().get(i).toString().substring(txtfilesearcher.getList().get(i).toString().lastIndexOf("/")+1,txtfilesearcher.getList().get(i).toString().lastIndexOf(".")) %>" title="" ><font color="#FF712D"><%=fathernode.getName().toString() %>
											&nbsp;&nbsp;&nbsp;&nbsp;[<%=fathernode.getAttribute().getFileno()==null?"":fathernode.getAttribute().getFileno() %>]</font></a>
							...
							<%
									//String[] a = new String["关于"];
									String print=(String)txtfilesearcher.getList2().get(i);
									//print=print.replace("\"","").replace("function show()","").replace("dname=documentgetElementById(","").replace("justlr","");
									//print=print.replace("\"","").replace(")","").replace("getElementsByTagName(","").replace("div","");
									print=print.replace("body,td,table {MARGIN: 0px;font-size: 14px;line-height: 160%;color: #333333;SCROLLBAR-FACE-COLOR: #E4F5FC;SCROLLBAR-HIGHLIGHT-COLOR: #ffffff;SCROLLBAR-SHADOW-COLOR: #4E9BC9;SCROLLBAR-3DLIGHT-COLOR: #A6D2F7;SCROLLBAR-ARROW-COLOR: #A6D2F7;SCROLLBAR-TRACK-COLOR: #E0F0FD;SCROLLBAR-DARKSHADOW-COLOR: #E0F0FD;SCROLLBAR-BASE-COLOR: #F2F2F2;text-decoration: none;}","");
									if(print.indexOf("function")>0)
									print = print.substring(0,print.indexOf("function")-1);
									if(print.indexOf("{ var")>0)
									print = print.substring(0,print.indexOf("{ var")-1);
									if(print.indexOf("}")>0)
									print = print.substring(print.indexOf("}")+1,print.length());
									if(print==null){
						 	...
						 	<br>
								<center>
									<%
										if(dang>1){
									%>
											<A HREF="law_query.jsp?dang=<%=dang-1%>&searchWord=<%=queryStr%>" class="fa">
									<%
											out.print("上一页");	
									%></A>
									<%
										...
										%>
												<A <%if(j==dang)out.print("style='color:#CC0000'");%> HREF="law_query.jsp?dang=<%=j%>&searchWord=<%=queryStr%>">
										<%
												out.print(j);	
										%></A>
										<%				
									 ...
									 <%
										if(dang<zongye){
									 %>
											<A HREF="law_query.jsp?dang=<%=dang+1%>&searchWord=<%=queryStr%>">
									 <%
											out.print("下一页");	
										}
									 %></A><br><br><br>
