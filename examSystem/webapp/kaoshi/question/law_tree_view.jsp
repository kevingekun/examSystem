...
<%
	...
	String status = RequestUtils.getString(request, "status", "1");
    ...
    Node nodeBase =  tree.getRootCatalog(Node.getNodeType(cname),user.getUserType());
...
private String createMenu(Node node,String http)  {
	...
	str.append(menu + "[1] = '<tr onmouseover=javascript:this.style.background=\"#B6C9D8\" onmouseout=javascript:this.style.background=\"#E0E9F1\" onclick=doAction(\"policy_list_view.jsp?nodeid=" + node.getId() + "\") class=\"word10\"><td style=\"height:16px; cursor:hand\" >&nbsp;&nbsp;文档列表</td></tr>'\n");
...
<script language="JavaScript">
function doAction(u){
	parent.content_body.location.href=u;
}
function closefrm(){
	parent.fr.cols="0,*";
}
...
function reloadWin(){
	document.location.href="law_tree.jsp";
}
function noright(){
	alert("抱歉，您未授权访问该栏目!");
}
function show(m, obj) {
	...
	doAction("policy_list_view.jsp?nodeid="+m);
}
...
<DIV id=loader>
<DIV align=left><img src="images/loading.gif"  align="absmiddle">页面正在加载中 ...</DIV>
</DIV></DIV>
<DIV>
...
menusite<%=nodeBase.getId()%>[1] = '<tr onmouseover=javascript:this.style.background="#B6C9D8" onmouseout=javascript:this.style.background="#E0E9F1" onclick=doAction("policy_list_view.jsp?nodeid=<%=nodeBase.getId()%>")  class="word10"><td style="height:16px; cursor:hand" ></td></tr>'
